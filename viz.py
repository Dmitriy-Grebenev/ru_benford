...
def plot_digs(df, x, y_Exp, y_Found, N, figsize, conf_Z, text_x=False,
    ...
    if len(x) > 10:
        ...
        rotation = 0
    fig, ax = plt.subplots(figsize=figsize)
    plt.title('Сравнение ожидаемых и найденных распределений', size='xx-large')
    ...
    if conf_Z is not None:
        ...
        ax.fill_between(x, upper, lower, color=COLORS['s'],
                        alpha=.3, label='Доверительный интервал')
    else:
        c = COLORS['m']
    ax.bar(x, y_Found * 100., color=c, label='Найдено', zorder=3, align='center')
    ax.plot(x, y_Exp * 100., color=COLORS['s'], linewidth=2.5,
            label='Бенфорд', zorder=4)
    ax.set_xticks(x)
...
def plot_sum(df, figsize, li, text_x=False, save_plot=None, save_plot_kwargs=None):
    ...
    plt.title('Сравнение ожидаемых и найденных сумм')
    plt.xlabel('Цифры')
    plt.ylabel('Суммы')
...
def plot_ordered_mantissas(col, figsize=(12, 12),
    ...
    ax.plot(x, col.sort_values(), linestyle='--',
            color=COLORS['s'], linewidth=3, label='Мантиссы')
...
def plot_mantissa_arc_test(df, gravity_center, grid=True, figsize=12,
    ...
    ax = plt.subplot()
    ax.set_facecolor(COLORS['b'])
    ax.scatter(df.mant_x, df.mant_y, label="Тест дуги мантисс",
               color=COLORS['m'])
    ...
    ax.set_title("Тест дуги мантисс")
